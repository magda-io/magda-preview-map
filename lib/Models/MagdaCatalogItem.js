...
/**
 * try to locate the WMS layer by its name.
 * If `layerName` is empty or can't find in items, just return the first available layer.
 *
 * @param {String} layerName the name of layer
 * @param {CatalogMember[]} items a list of CatalogMembers converted from Wms Layer data
 * @return {CatalogMember}
 */
MagdaCatalogItem.findWmsLayerByName = function (layerName, items) {
  ...
  for (var i = 0; i < items.length; ++i) {
    if (items[i].toLowerCase().trim() === layerName.toLowerCase().trim()) {
      return items[i];
    }
  }
  return items[0];
    ...
    if (defined(layerName)) {
      ...
      newItem = newGroup.load().then(function () {
        if (newGroup.items.length === 0) {
          ...
          // try to locate the layer by layer name
          // if selectedWmsLayerName is empty or can't find just return the first one
          return MagdaCatalogItem.findWmsLayerByName(
