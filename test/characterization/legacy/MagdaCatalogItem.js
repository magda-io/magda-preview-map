...
function MagdaCatalogItem(terria) {
  ...
  /**
   * Optional. Selected WMS Layer Name.
   * When the value is empty, the first layer will be selected by default.
   * Otherwise, the selected layer will be returned for rendering.
   * @type {String}
   */
  this.selectedWmsLayerName = "";

  /**
   * Optional. Selected WFS Type Name.
   * When the value is empty, the first FeatureType will be selected by default.
   * Otherwise, the selected FeatureType will be returned for rendering.
   * @type {String}
   */
  this.selectedWfsFeatureTypeName = "";

  /**
   * Magda storage api URL. Only used for generating internal storage data file access URL
   * @type {String}
   */
  this.storageApiUrl = "";

  /**
   * Gets or sets the name of the internal storage bucket that is used to store the data file of the distribution.
   * @type {String}
   */
  this.datasetBucket = "magda-datasets";
   ...
   * @type {RegExp}
   */
  this.esriMapServerDistributionFormat = /^esri (rest|MapServer)$/i;
   ...
   * @type {RegExp}
   */
  this.esriFeatureServerDistributionFormat = /^esri (rest|FeatureServer)$/i;
...
MagdaCatalogItem.defaultUpdaters.wmsDistributionFormat =
  createRegexDeserializer("wmsDistributionFormat");
MagdaCatalogItem.defaultUpdaters.wfsDistributionFormat =
  createRegexDeserializer("wfsDistributionFormat");
MagdaCatalogItem.defaultUpdaters.kmlDistributionFormat =
  createRegexDeserializer("kmlDistributionFormat");
MagdaCatalogItem.defaultUpdaters.csvDistributionFormat =
  createRegexDeserializer("csvDistributionFormat");
MagdaCatalogItem.defaultUpdaters.esriMapServerDistributionFormat =
  createRegexDeserializer("esriMapServerDistributionFormat");
MagdaCatalogItem.defaultUpdaters.esriFeatureServerDistributionFormat =
  createRegexDeserializer("esriFeatureServerDistributionFormat");
MagdaCatalogItem.defaultUpdaters.geoJsonDistributionFormat =
  createRegexDeserializer("geoJsonDistributionFormat");
MagdaCatalogItem.defaultUpdaters.czmlDistributionFormat =
  createRegexDeserializer("czmlDistributionFormat");
...
MagdaCatalogItem.defaultSerializers.wmsDistributionFormat =
  createRegexSerializer("wmsDistributionFormat");
MagdaCatalogItem.defaultSerializers.wfsDistributionFormat =
  createRegexSerializer("wfsDistributionFormat");
MagdaCatalogItem.defaultSerializers.kmlDistributionFormat =
  createRegexSerializer("kmlDistributionFormat");
MagdaCatalogItem.defaultSerializers.csvDistributionFormat =
  createRegexSerializer("csvDistributionFormat");
MagdaCatalogItem.defaultSerializers.esriMapServerDistributionFormat =
  createRegexSerializer("esriMapServerDistributionFormat");
...
MagdaCatalogItem.defaultSerializers.geoJsonDistributionFormat =
  createRegexSerializer("geoJsonDistributionFormat");
MagdaCatalogItem.defaultSerializers.czmlDistributionFormat =
  createRegexSerializer("czmlDistributionFormat");

freezeObject(MagdaCatalogItem.defaultSerializers);

/**
 * Convert the resource url to Magda storage API access url if it's a [pseudo storage api resource URL](https://github.com/magda-io/magda/issues/3000).
 * Otherwise, return the original resource URL.
 *
 * @param {String} resourceUrl the resource download url
 * @param {String} datasetBucket the name of the internal storage bucket that is used to storage internal dataset files.
 * @param {String} storageApiUrl the url of storage API
 * @return {String} the runtime accessible url of the resource
 */
MagdaCatalogItem.getStorageApiResourceAccessUrl = function (
  resourceUrl,
  datasetBucket,
  storageApiUrl
) {
  const uri = URI(resourceUrl);
  if (uri.protocol() === "magda" && uri.hostname() === "storage-api") {
    ...
    const accessUri = URI(storageApiUrl);
    const segments = accessUri
      .segmentCoded()
      .concat([datasetBucket, datasetId, distributionId, fileName]);
...
/**
 * try to locate the WMS layer or WFS feature type by its name.
 * If `name` is empty or can't find in items, just return the first available item.
 *
 * @param {String} name the name of group item
 * @param {CatalogMember[]} items a list of CatalogMembers converted from Wms Layer data or WFS feature type
 * @param {Boolean} isWms whether items are WMS items
 * @return {CatalogMember}
 */
MagdaCatalogItem.findWmsWfsGroupItemByName = function (name, items, isWms) {
  ...
  for (var i = 0; i < items.length; ++i) {
    var itemName = isWms ? items[i].layers : items[i].typeNames;
    if (
      itemName &&
      typeof itemName === "string" &&
      itemName.toLowerCase().trim() === name.toLowerCase().trim()
    ) {
      return items[i];
    }
  }
 ...
 * @param {Object} [options.datasetBucket] the name of the internal storage bucket that is used to storage internal dataset files.
 * @param {String} [options.storageApiUrl] Magda storage api URL. Only used for generating internal storage data file access URL.
 * @param {String} [options.selectedWmsLayerName] Selected WMS Layer Name. Optional.
 * @param {String} [options.selectedWfsFeatureTypeName] Selected WFS Feature Type Name. Optional.
...
MagdaCatalogItem.createCatalogItemFromDistribution = function (options) {
  ...
  baseUrl = MagdaCatalogItem.getStorageApiResourceAccessUrl(
    baseUrl,
    options.datasetBucket,
  ...
  function getQueryParameterValueWithPossibleKeys(query, keys) {
    if (!query) {
      return undefined;
    }
  ...
  function getWfsTypeNames(query) {
    return getQueryParameterValueWithPossibleKeys(query, [
      "typeNames",
      "TYPENAMES",
      "typeName",
  ...
  if (isWms || isWfs) {
    ...
    let layerOrTypeName = getQueryParameterValueWithPossibleKeys(params, [
      "layers",
      "LAYERS",
      "layer",
      "LAYER",
    ...
    if (
      defined(layerOrTypeName) ||
      options.selectedWmsLayerName ||
      options.selectedWfsFeatureTypeName
    ) {
      newItem = isWms
        ? new WebMapServiceCatalogItem(options.terria)
        : new WebFeatureServiceCatalogItem(options.terria);
      if (isWms) {
        newItem.layers = options.selectedWmsLayerName
          ? options.selectedWmsLayerName
          : layerOrTypeName;
      } else {
        newItem.typeNames = options.selectedWfsFeatureTypeName
          ? options.selectedWfsFeatureTypeName
          : layerOrTypeName;
      }
      newItem.url = url;
      ...
      newItem = newGroup.load().then(function () {
        if (newGroup.items.length === 0) {
          ...
          // try to locate group item by WMS layer name or WFS Feature Type Name
          // if selectedWmsLayerName (or selectedWfsFeatureTypeName) is empty or can't find just return the first one
          return MagdaCatalogItem.findWmsWfsGroupItemByName(
            isWms
              ? options.selectedWmsLayerName
              : options.selectedWfsFeatureTypeName,
            newGroup.items,
  ...
  return when()
    .then(function () {
      if (defined(that.distributionId)) {
        ...
        throw new TerriaError({
          sender: that,
          title: "Error retrieving dataset record",
    ...
    .then(function (distributionsToConsider) {
      ...
      for (var i = 0; i < distributionsToConsider.length; ++i) {
        var catalogItemCreatingAttempt =
          MagdaCatalogItem.createCatalogItemFromDistribution({
            terria: that.terria,
            ...
            esriMapServerDistributionFormat: that.allowEsriMapServer
              ...
              : undefined,
            esriFeatureServerDistributionFormat: that.allowEsriFeatureServer
              ? that.esriFeatureServerDistributionFormat
              : undefined,
            ...
            datasetBucket: that.datasetBucket,
            selectedWmsLayerName: that.selectedWmsLayerName,
            selectedWfsFeatureTypeName: that.selectedWfsFeatureTypeName,
            storageApiUrl: that.storageApiUrl
              ? that.storageApiUrl
              : URI(that.url)
                  .segmentCoded(
                    URI(that.url)
                      .segmentCoded()
                      .concat(["api", "v0", "storage"])
                  )
                  .toString() + "/",
      ...
      return when.any(catalogItemCreatingAttempts).otherwise(function (e) {
        var genericError = new TerriaError({
          ...
          message: defined(that.distributionId)
            ? "The dataset does not have a distribution with the ID " +
              that.distributionId +
              " or it does not have a supported format."
            : "The dataset does not have any distributions with a supported format.",
        });
